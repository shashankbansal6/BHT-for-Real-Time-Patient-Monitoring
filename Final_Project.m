...
for i = 1:8
    for j = i+1:9
        for k = 1:7
            if(length(patient(i).all_data) <= length(patient(j).all_data))
                A = patient(i).all_data;
                B = patient(j).all_data(:, 1:length(A));
                R = corrcoef(A(k,:), B(k,:));
            
            else 
                B = patient(j).all_data;
                A = patient(i).all_data(:, 1:length(B));
                R = corrcoef(A(k,:), B(k,:));
            end
